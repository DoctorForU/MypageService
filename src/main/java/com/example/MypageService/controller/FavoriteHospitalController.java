...
public class FavoriteHospitalController {
    ...
    @PostMapping("/favorite-hospitals")  // 로그인 후 mypage에 들어갈 때 -> 자동으로 렌더링되어서 관심병원 리스트가 상단에 나오게 할 때 부르는 컨트롤러
    public ResponseEntity<List<FavoriteHospitalResponse>> getFavoriteHospitals(@RequestBody String userId) {  // 로그인 후 mypage에 들어갈 때 -> 자동으로 렌더링되어서 관심병원 리스트가 상단에 나오게 할 때 부르는 컨트롤러
        List<FavoriteHospitalResponse> favoriteHospitals = favoriteHospitalService.getFavoriteHospital(userId);
